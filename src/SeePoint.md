...
11                      sleep(10);
```
先得到`a`的地址：`0x6009c8`，接着用`watch *(int*)0x6009c8`设置观察点，可以看到同`watch a`命令效果一样。 观察点可以通过软件或硬件的方式实现，取决于具体的系统。但是软件实现的观察点会导致程序运行很慢，使用时需注意。
...
查看所设置的断点也可以用控制断点的命令来控制。如 `disable`、`enable`、`delete`等

- **Tips-24:** 设置观察点只针对特定线程生效
...
10                      a++;
```
可以看到，使用`aw a`命令（`aw`是`awatch`命令的缩写）以后，每次读取或改变a的值都会让程序停下来。
